...
export default function WeeklyGrid({ days, onDayClick, onTopicClick }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-7 gap-1 h-full">
            {days.map((day) => {
                    ...
                    >
                        ...
                        <div className="flex-grow p-2 overflow-y-auto custom-scrollbar space-y-2">
                            {day.temas && day.temas.length > 0 ? (
                                day.temas.map((tema) => (
                                    <CalendarTopicCard key={tema.id} topic={tema} onClick={onTopicClick} />
                                ))
                            ) : (
